...
| 职业 | 当前完成 | 剩余 | 主要缺口 |
|---|---:|---:|---|
| 亚马逊 Amazon | 72% | 28% | Dodge/Avoid/Evade 被动判定、完整弓系伤害/动画帧仍待补齐 |
...
已完成 **亚马逊弓系专项：Guided Arrow、Strafe 与 Pierce 命中链**：`SrvDo010` 导引目标/失效后直飞、`SrvDo012` 按范围和目标数量逐箭发射，以及 Pierce 命中去重和继续飞行均已接入并覆盖测试。
下一项是 **Dodge/Avoid/Evade 被动判定**，随后再补齐亚马逊弓系的完整伤害/动画帧细节。
