...
- [x] ~~对边界候选执行局部碰撞 BFS~~
  ...
  - 1.10f 结果：`boundaryBfsResolved=5`、`boundaryBfsBlocked=68`、
    ...
    在入口关闭状态下不可达。
  - 进一步关联原生对象记录后，68 个静态阻断候选中有 61 个附近存在原生对象，其中 19
    个可解析为 `Objects.txt IsDoor`；仅 7 个没有附近对象记录，列为下一轮优先核对项。
  - 详细候选写入连续性 manifest，后续可按 Level/RoomEx 精确复现，不修改生产碰撞。
- 验证：真实 1.10f 隐藏离屏连续性任务、桌面编译通过。

当前下一项：优先核对剩余 7 个“无附近对象但 BFS 阻断”接口和 55 个几何不可判定接口，
再把门状态/对象碰撞快照接入入口专项验收。
