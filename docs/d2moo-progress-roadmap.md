...
- [x] Inventory/箱子金币输入不再调用平台原生 `Gdx.input.getTextInput`。新增
  ...
  同一 MPQ 命令复核。
- [x] 商人购买确认框已改用原版 `MENU/boxpieces.dc6` 边框；标题、商品名、价格及纵向
  是/否按钮按原版对话框布局显示，按钮悬停/按下使用原版蓝色文字状态；左键确认、确认后
  放到鼠标光标以及右键直接购买的行为保持不变。
...
- 最近已完成并可由代码/专项测试复验：商店左键购买到光标、右键直接入包及服务端同步；
  ...
  伤害与 AR 缩放；怪物索敌范围；物品/药水/金币拾取；Automap `.map`/`.ma0-.ma3`
  读写；HackMap BMP → 内存 `CellFile` 链路；A2Q6 插杖、Duriel/Tyrael 门和重连恢复。
- 本轮补充 A2Q6 插杖状态门槛：已进入 Duriel 流程、完成 Tyrael 结算或已前往 Act III 的
  角色，即使背包残留 `hst` 也不能再次激活法杖孔；对应状态转换有单元测试。
- 本轮继续收紧 A2Q6 Tyrael 302：网络消息校验和服务端处理都要求角色已有 Duriel
  击杀后的持久化进度；已完成过的角色仍可幂等重放消息以恢复/复用回城门，不会提前开门。
- A2Q6 网络对话白名单已补齐 Jerhyn 442 与 Meshif 450：分别要求 `LEFT_TOWN` 和
  `ENTERED_AREA`，防止伪造对话包跳过 Tyrael/任务状态转换。
- A2Q6 442/450 在断线重连后允许安全重放：已进入后续状态的重复对话只复用幂等状态，
  不会重复发奖励、重复开门或回退任务记录。
...
- 玩家交易基础状态机开始收紧：交易栏拒绝重复物品 ID、空尺寸和越界尺寸，避免同一物品
  被重复提交后在完成阶段产生复制；新增纯 Java 回归，真实网络原子转移仍待后续接线。
- 玩家交易确认现在必须经过独立 `TradeAuthority` 两阶段边界：先验证物品/金币/容量，
...
- [x] 接通佣兵升级回调：等级提升后刷新运行时/存档属性、技能、最大生命和经验字段，
  并重新聚合佣兵装备，避免升级覆盖装备词缀或只改变等级不改变战斗数值。
- [x] 补齐佣兵复活后的同步：复活实体恢复后重新应用 Hireling 原生属性、技能、经验和装备
  聚合；死亡佣兵保持 0 生命，复活后同时更新运行时实体与 D2S 生命字段，避免只清除死亡标记
  却继续使用旧的战斗属性。
- [x] 统一腰带容量校验：手动光标放入腰带、本地调用和 D2GS `ItemMoveValidator` 均按当前
  腰带类型的 1/2/3/4 行容量检查，并拒绝非药水物品或超出当前腰带行数的陈旧请求。
- [x] 修正套装属性聚合时序：装备套装物品先更新已装备件计数再重算，卸下时先移除计数再
  重算，避免套装部分/完整奖励延迟一轮或在卸下后残留。
- [x] 修正权威近战力量伤害倍率：按 1.10f 的 100 力量 = +100% 武器伤害处理；此前错误地
  使用 `strength / 100`，使 100 力量只增加 1%。新增确定性 CombatSystem 回归覆盖。
- [x] 修正 ToHit 的模式防御聚合：`armorclass_vs_missile` 与 `armorclass_vs_hth` 现在分别
  按远程/近战攻击加到基础防御，而不是仅在基础防御为零时作为替代值；新增远程/近战命中率
  差异回归覆盖。
- [x] 接入 `item_tohit_percent_vs_monster`：该攻击率百分比只对怪物目标生效，不再错误影响
  PvP/玩家目标；新增玩家对怪物与玩家目标的命中率隔离测试。
- [ ] 战斗主体仍需继续补齐原版 `D2DamageStrc` 的完整 ToHit、伤害、抗性、状态和投射物分支；
  当前可在无真实客户端条件下继续使用纯 Java 与 D2GS 离屏测试推进。

## 2026-09-21 地图预热、物品流程与投射物对齐（本轮完成）

- [x] 城镇按出口方向预生成野外入口；靠近野外区域边界时预生成下一地图入口附近房间、怪物和
  物件。延迟 RoomEx 中的传送点会在传送前主动实例化。
- [x] 保留 D2MOO 选择的具体 DT1 来源，避免重复 tile id 令洞穴入口等图块在悬停/动画替换时
  切错资源；碰撞后按实际位移更新移动模式，修复阻挡时原地跑步。
- [x] 建立原版物品类型 Treasure Class，恢复装备掉落；补齐 `Alt` 地面标签、背包开启拾取到
  光标、满包弹起和掉落声音、商人出售及 Automap 按钮。
- [x] 红蓝药水按 `Misc.txt` 渐进恢复，紫药水立即恢复；本地与 D2GS 均运行恢复系统。
- [x] Act I Horadric Malus 任务按原版限制为角色等级 8；NPC 提示、任务日志和服务器对话统一。
- [x] 普通弓/弩 Attack 创建权威箭矢并扣除弹药；穿透按 1.10f 的装备+技能概率在发射时预掷，
  最多穿透 4 次，不再把 `Missiles.txt.Pierce` 误当成固有 100%。
- [x] 核心与 D2GS 编译、投射物/投掷/骨矛定向测试通过。亚马逊完整测试 21 项中 20 项通过，
  剩余既有 Immolation Arrow 持续火焰场失败待单独处理。

详细记录见 [`codex-handoff-2026-09-21.md`](codex-handoff-2026-09-21.md)。
`game.log` 与 `_tmp_run.ps1` 继续保留为本地未跟踪诊断文件。

## 2026-09-20 输入、战斗、资源、Automap 与 Help 对齐（本轮完成）

- [x] 重整悬停选择、点击队列和交互范围；NPC、箱子、掉落物会在远距离点击后自动靠近并
  单次交互，近战攻击也统一按实际武器/技能距离判定。
- [x] 对齐普通点击与 Shift 强制施法：近战右键未点中目标时移动，Shift+左/右键原地播放
  技能动作，未实际命中或施展不扣魔力。
- [x] 修正体力、魔力和跑步状态：城镇奔跑不耗体力，耗尽后不改变跑步开关，`Ctrl` 临时跑步，
  魔力自动恢复且不足时技能图标变红。
- [x] 补齐怪物近战命中/距离、追击脱离和区域生成；城镇加载时预激活出口附近房间，Fallen
  Shaman 导弹失效后从场景与 Automap 清理。
- [x] 修正 Automap 探索读取时机和裸露道路地面显示；修正体力条、经验条、角色/技能/升级按钮、
  技能说明与依赖禁用、角色面板战斗数值和升级反馈。
- [x] 新增 640x480 原版风格 Help Overlay，`H` 切换、`Esc` 关闭，宽屏居中且阻止背景点击
  穿透；布局参考 `dark-magic` 和 `OpenDiablo2`。
- [x] 核心编译和 Help/UI 定向测试通过。完整 `:core:test` 运行 1744 项、109 项失败，主要为
  既有测试资源缺失及战斗数据断言，因此完整测试集仍需后续清理，不能标记为全绿。

详细变更、验证范围与后续入口见
[`codex-handoff-2026-09-20.md`](codex-handoff-2026-09-20.md)。本轮提交以该文件所在
提交的 `HEAD` 为准；`game.log` 与 `_tmp_run.ps1` 保持本地未跟踪，不加入提交。
...
- 本轮完成的是权威插杖/红门最小闭环；七墓 Level/Arcane Symbol 选择、缺失 DS1
  预设的稳定 fallback、Duriel/Tyrael/Jerhyn/Meshif 的持久化状态已有代码和专项测试。
  插杖入口现在还会校验 A2Q6 记录，拒绝在 Duriel 流程已开始后用残留 `hst` 重复激活。
  下一项优先补齐七墓真实预设/对话状态的更多离屏回归，再回补 A2Q3 NPC 奖励对话。
...
- Blessed Hammer 原生权威链已完成首轮：`SrvDo073` 创建一枚与点击方向无关的 77 点
  外扩螺旋导弹，按 120 frame 生命周期运行；每个螺旋折线段独立执行地图屏障射线与
  单位扫掠，同一目标仅命中一次而 `CollideKill=0` 允许继续命中其他目标。基础魔法伤害、
...
- Resist Fire、Resist Cold、Resist Lightning 与 Salvation 已完成原生 `SrvDo065` 光环：
  范围、50-frame 周期、目标过滤和抗性公式均读取 1.10f `Skills.txt`。三种单抗的激活
...
- 圣骑士 Charge 已接入原生 `SrvSt31/SrvDo067`：起手校验敌对目标并按 `Param1` 安装
  ...
  编译通过。
- 圣骑士 Vengeance 已接入原生 `SrvSt35/SrvDo002`：起手按 `calc1/calc2/calc3` 同时生成
  火/冰/电三种元素伤害包并保存命中结果，关键帧只消费一次，成功后仅轮换 HitClass
...
- 法师 Meteor 已接入原生 `SrvDo028` / `SrvHit14`：施法只创建目标点
...
- 德鲁伊 Fissure 已接入原生 `SrvDo028` 控制链首轮：不再误走 Meteor，服务端在目标点
  创建 Fissure 控制导弹，按其 SubMissile/HitSubMissile 数据周期铺设火焰裂缝子导弹，
  统一保存火焰伤害快照、持续时间和 DamageRate，并通过权威导弹同步多人客户端。新增
  `DruidFissureIntegrationTest`；Fissure 的真实资源观感仍需离屏 MPQ 验收。
...
| P2 | NPC 买卖、修理、赌博、雇佣 | 4% | 83% | 17% | 0.7% | 买卖/修理/赌博、左右键分流和稳定身份重连已接通；雇佣/复活及原生刷新仍待补 |
| P2 | Party、敌对、玩家交易和多人同步 | 5% | 63% | 37% | 1.9% | 交易基础状态机和重复物品防护已有；物品/金币原子转移、网络协议和重连仍待补 |
| P2 | D2S 存档、角色创建和状态持久化 | 4% | 99% | 1% | 0.04% | 完整 section、`.key` 和 Automap sidecar 已有；剩余为原版客户端外部识别与极少数边界存档 |
...
- 2026-09-11：完成法师 Meteor 原生落点/爆炸链。`SrvDo028` 仅创建目标点
  `meteorcenter`，服务端固定等待 60 帧后按 `SrvHit14` 的 `aurarangecalc` 结算一次火焰
  冲击，并用 18 个原生偏移生成 `meteorfire` 持续区域；区域持续时间按 `Param3/Param4`
  读取，火焰精通/穿透/抗性/吸收/PvP/死亡事件沿用权威固定元素链。新增
  `SorceressMeteorIntegrationTest` 覆盖表项、延迟、即时冲击、18 个火场和生命周期；
  定向法师回归通过，下一项为 Thunder Storm。
...
- 2026-09-10：完成圣骑士 Vengeance `SrvSt35/SrvDo002` 首轮移植。按当前武器物理范围
...
- [x] ~~完成 Blessed Hammer 原生导弹、碰撞与 Concentration 特殊增伤首轮~~
  - 对齐 D2Common `PATHTYPE_BLESSEDHAMMER` 的 77 点固定全局方向外扩螺旋与 120 frame
    生命周期；每个折线段分别执行地图 `CollideType=3` 屏障射线和单位扫掠，导弹可连续
    命中不同目标但对同一目标终身去重，墙体命中立即终止。
  - 对齐 1.10f 基础魔法伤害、Vigor/Blessed Aim 硬点协同和施法时 Concentration
    `damagepercent * Param1 / 8` 快照；`SrvDmg05` 的不死/恶魔各 50% 加成按原生顺序在
    随机伤害之后、抗性/吸收/PvP 之前执行，两种类型可累加。
  - 单机 monsters-only ServerSkillSystem 也创建权威导弹，客户端表现层检测该路径后复用
    权威实体，避免本地或多人双导弹。23 个联合测试套件共 107 个用例通过，D2GS 编译
...
- [x] ~~完成圣骑士抗性光环与硬点最大抗性首轮~~
  - Resist Fire、Resist Cold、Resist Lightning 与 Salvation 统一通过原生 `SrvDo065`
    数据驱动范围、`perdelay=50`、目标过滤和 `dm34` 抗性公式。
  - 三种单抗的选中光环最大抗性与永久 `PassiveState` 分层保存；永久部分只读取角色硬点，
    不受装备 `+skills` 影响，施法者与队友不会重复计算。
  - 普通攻击和固定元素伤害均聚合状态最大抗性；按 D2MOO `SUnitDmg.cpp` 修正玩家总抗
    达到 100 时仍受 75～95 上限约束，而怪物 100+ 抗性继续按免疫处理。
  - 5 个专项/联合测试套件共 44 个用例、`:server:d2gs:compileJava` 与真实 1.10f
    `:desktop:offscreenCamp` 均通过，输出 `[OFFSCREEN_CAMP] result=PASS`；未修改
    FlatBuffers schema，无生成网络文件差异。
...
### 2026-09-11 RoomEx 激活快照一致性（已完成）

- [x] ~~断言房间激活变化不修改已生成 Automap cell~~
  - `AutomapRoomProjectionIntegrationTest` 在玩家跨 RoomEx 移动前后，对 floor/wall/object
    cell 数量和世界坐标做快照比较；激活状态只改变探索掩码与实体可见范围。
- 验证：Automap 全套无资源测试与核心编译通过；真实 1.10f 画面测试仍按当前条件跳过。
...
### 2026-09-11 RoomEx 边界局部 BFS 复核（已完成初版）

- [x] ~~对边界候选执行局部碰撞 BFS~~
  - 对 128 个直接采样不到过渡的接口，在两个 RoomEx 联合包围盒内执行有限 4 邻域 BFS，
    区分存在门口/楼梯路径、静态碰撞确实阻断和几何范围无法判断。
  - 1.10f 结果：`boundaryBfsResolved=5`、`boundaryBfsBlocked=68`、
    `boundaryBfsUncheckable=55`。这些数据仍不直接判定失败，因为 D2MOO 的 `pRoomsNear`
    是激活/视野邻接，不保证两个房间存在静态直通路径；门、楼梯和对象碰撞可能使静态 BFS
    在入口关闭状态下不可达。
  - 对象初步统计口径覆盖全部 128 个直接边界候选，后续已按 BFS 结果与 Warp 房间重新
    分类，准确结果见下一项，避免把无法判断的接口误算为“已由对象解释”。
  - 详细候选写入连续性 manifest，后续可按 Level/RoomEx 精确复现，不修改生产碰撞。
- 验证：真实 1.10f 隐藏离屏连续性任务、桌面编译通过。

### 2026-09-11 Warp 房间阻断严格分类（已完成）

- [x] ~~按 BFS/对象/Warp 房间交叉分类边界候选~~
  - 68 个 BFS 静态阻断接口分为：2 个附近有门、25 个附近有其他原生对象、41 个没有
    附近对象；后者属于普通 `pRoomsNear` 视野邻接，不能据此判断应当直通。
  - 其中真正涉及 Warp 所在房间的静态阻断接口为 8 个，8 个均存在附近原生对象；
    `boundaryWarpBare=0`，未发现“入口房间被静态墙封死且没有门/楼梯/入口对象解释”。
  - `boundaryWarpBare == 0` 已提升为连续性任务的严格 PASS 条件，未来任意固定种子生成
    出无对象解释的 Warp 阻断会直接使离屏任务失败。
- 验证：真实 1.10f 隐藏连续性扫描通过，完整报告输出 Level/RoomEx/objectKind/Warp 标记。

当前下一项：对 8 个 Warp 房间对象阻断接口验证对象打开/切换状态后的动态碰撞更新，确认
门、楼梯和入口对象不会永久保留 `BLOCK_WALK`。
...
### 2026-09-11 升级属性点/技能点 UI 与多人分配链路（已完成代码修复）

- [x] ~~修复技能树加点入口~~：未学习技能不再因 `sLvl == 0` 被禁用；有可用技能点时
  所有本职业技能入口可点击，等级、前置和职业限制仍由服务端权威校验。
- [x] ~~补齐属性点入口~~：力量、敏捷、体力、精力四个 `+` 按钮继续显示可用点数；
  单机直接分配，多人发送新的幂等 `SpendStatPointRequest`，服务端返回权威值并同步。
- [x] ~~修复升级后 UI 不刷新~~：`SpellsPanel` 轮询 `PlayerP` 同步到 `CharData` 的
  `newskills`，升级后立即刷新点数标签和按钮状态；属性面板继续实时刷新 `statpts`。
- 新增 `StatRequest.fbs`、生成的 FlatBuffers 请求/结果、`StatPointRequestCache`，以及
  属性点权威/网络回归测试。技能点原有服务端校验与幂等逻辑保持不变。
- 验证：`:core:compileJava`、`:server:d2gs:compileJava` 以及属性点、技能点和网络传输
  回归测试通过。
...
### 2026-09-12 地面金币点击直拾取（已完成代码修复）

- [x] ~~单机金币点击直接计入携带金币~~：`ClientItemManager` 识别 `gld`，不再把金币
  放到鼠标；携带上限沿用原生等级上限，容量不足时只拾取可容纳数量并保留地面余量。
- [x] ~~联机继续使用服务端权威交易~~：客户端只发送 `GROUND_TO_CURSOR`，由 D2GS
  校验距离、归属、revision 和幂等；新增缺失网络实体/物品组件的拒绝诊断，不在客户端
  预测钱包或提前删除地面实体。
- [x] ~~金币 UI 实时刷新~~：背包金币标签每次绘制从当前权威 `Stat.gold` 更新，单机
  直拾取和多人快照应用后无需关闭再打开面板。
- 新增 `[GROUND_PICKUP]` 点击、请求、授予和拒绝日志；新增单机完整拾取、部分拾取、
  满钱包不消费地面堆三条 ECS 回归。
- 验证：金币/权威移动/客户端 revision 共 25 个相关用例、`:core:compileJava`、
  `:server:d2gs:compileJava` 全部通过；真实 1.10f `:server:d2gs:headlessReconnectGroundLoot`
  通过，确认 20 金币堆只收取 5、权威余量 15、争抢拒绝校正和断线重连归属保持。

当前下一项：运行真实 D2GS/隐藏双客户端金币拾取验收，确认完整拾取删除、部分拾取数量、
两端钱包/地面实体同步；随后复核最新日志中的点击延迟和卡顿，优先定位输入到权威 tick 的
排队延迟，当前 Chat 继续负责包括战斗在内的全部模块。

### 2026-09-12 地面点击输入边沿与模拟卡顿诊断（已完成代码修复）

- [x] ~~捕获短鼠标点击~~：`CursorMovementSystem` 在渲染帧捕获左键上升沿，在下一次
  25Hz 固定 Tick 消费；即使按钮在两个 Tick 之间释放，地面移动命令也不会丢失。
- [x] ~~保持 UI 与实体点击语义~~：消费时使用捕获的屏幕坐标重新检查两个 Stage；UI 点击
  不会误触发移动，已有 Hovered 实体继续走交互/攻击路径，未命中实体则直接转换为地面目标。
- [x] ~~增加延迟诊断~~：超过 30ms 的单 Tick 输出 `[SIM_SLOW]`，捕获命令等待超过 40ms
  输出 `[INPUT_QUEUE]`，用于区分输入采样延迟和模拟系统过载。
...
### 2026-09-12 `[SIM_SLOW]` 系统级热点诊断（已完成代码修复）

- [x] `SystemProfiler` 暴露最近一次系统耗时，`GameScreen` 的慢 Tick 日志现在附带前三个
  CPU Artemis 系统及耗时，例如 `systems=[Pathfinder=12.4ms, ...]`。
- [x] GPU 渲染系统从慢 Tick 排名中排除，避免把渲染帧耗时误判为模拟瓶颈；原有总耗时、
  30ms 阈值和限频行为保持不变。
- 验证：`:core:compileJava` 通过。
...
### 2026-09-12 修复点击目标后的标枪投掷丢失（已完成代码修复）

- [x] 点击队列命中目标但角色仍处于上一段攻击/投掷序列时，不再丢弃点击；原始坐标、捕获
  Tick 和时间戳会被恢复到队列，待模拟系统可中断后重试。
- [x] 标枪/投掷武器的释放距离与 `Pathfinder` 的原生停步阈值统一为
  `RangeAdder + 3 + 1`，避免角色停在投掷范围外一格导致永远不生成投射物。
- [x] 增加点击队列恢复回归，确保重试不会改变点击坐标或 Tick。
- 验证：`PointerClickQueueTest`、`CursorMovementSystemTest`、`:core:compileJava` 通过。
...
### 2026-09-12 Act V A5Q5 远古人原生经验奖励（本轮完成）

- [x] 按 D2MOO `ACT5Q5_RewardPlayer` 接入普通/噩梦/地狱固定经验
  `1,400,000 / 20,000,000 / 40,000,000`，并以玩家当前等级完整经验区间为上限，
  保留原版“最多提升一级”的规则；99 级不再获得经验。
- [x] 奖励仍受原生 `20/40/60` 等级门槛约束；山顶合格玩家及其位于第五章的合格
  队友分别、幂等领取，不在第五章或已完成 A5Q5 的玩家不会重复获得。
- [x] 经验发放复用服务端权威 `ExperienceManager`，因此等级、生命/法力成长、属性点、
  技能点、经验条及持久化与普通击杀经验使用同一条路径，而不是任务系统另写一套数值。
- [x] `Act5AncientsQuestTest` 覆盖三难度常量、当前等级经验跨度封顶、异常跨度和满级
  零奖励。
...
### 2026-09-12 任务门户删除/重建实体快照桥接（本轮完成）

- [x] 新增 `headlessRebuildQuestPortal`：删除指定源区域的任务 Warp，A3 同时重置 Hell Gate
  的原生对象状态，A4/A5 删除动态视觉实体，然后执行一次权威 `world.process()`，由三套
  任务系统按记录自动重建。
- [x] `--require-quest-warp-dual` 现在在 A3Q6、A4Q2、A5Q4 成功门槛后都执行删除/重建，
  校验新的 Warp 实体仍存在，A3 Gate 为开启态，A4/A5 视觉对象存在，再继续做双客户端
  可见性、幂等重放和过期源区域拒绝。
- [x] 通过 `:core:compileJava`、`:server:d2gs:compileJava`；真实资源场景未在当前环境
  执行，未新增 FlatBuffer 或生成网络文件。
...
### 2026-09-13 A5Q6 波次回归准备（本轮进行中）

- [x] 新增 `headlessBaalWaveDual` Gradle 入口，使用两个无窗口 TCP 客户端
  运行原生 A5Q6 波次、Baal 门槛、Worldstone Chamber 和奖励/重连路径。
- [x] 修正生成的 A5Q6 测试存档 waypoint：Act V 使用全局存档编号 30--38，
  不再把 Act I 的索引 0 误传给原生范围校验。
- [x] 波次清空增加一个固定 tick 的结算屏障，等待 Artemis 删除事件和敌对半径
  订阅在同一模拟 tick 完成后再请求下一波，避免“上一波最后一个实体已删但
  `isBaalThroneClear` 尚未更新”造成时序漂移。
- [ ] 当前离屏回归已确认第一波双客户端可见且位置一致；第二波服务端权威快照
  已生成（`authority=[1,4,1,3]`），但客户端观察器在实体基线差异下未捕获新实体，
  需要下一轮修正测试观察窗口/实体快照去重后再判定完整五波通过。
...
### 2026-09-13 A5Q6 波次观察器稳定性（本轮进行中）

- [x] 为离屏波次夹具增加权威玩家生命值初始化（同时设置 `maxhp` 与当前
  `hitpoints`），避免观察器因测试角色被波次 AI 击杀而中断同步验证。
- [x] `:server:d2gs:compileJava` 通过；前两波仍可双客户端观察到相同实体集合、
  类别和坐标。
- [ ] 当前完整五波运行仍需继续定位：在第三波前后出现观察进程长时间等待，需
...
### 2026-09-14 A4Q1 Tyrael 对话接线（本轮完成部分）

- [x] 新增 A4 网络消息校验：Tyrael2 的 `670` 初始化和 `676` 奖励消息现在可由
  D2GS 接受，并严格要求 A4Q1 的 pending 状态才能领取奖励。
- [x] 客户端 Cain4/Tyrael2 共用 A4 任务对话控制器，接入 1.10f 对应语音键；NPC
  菜单不再误走 Act1 任务控制器。
