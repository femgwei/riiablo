...
下一项建议进入 **服务端原生移动意图协议第二阶段**：把多人客户端的点击目标/目标实体/跑走状态作为输入发送，由 D2GS `Actioneer + Pathfinder + VelocityAdder` 独立生成权威路径和位移；停止接受客户端 COF 与绝对 Position 作为权威数据，并补延迟下连续转向、动态目标追踪、Warp 后旧输入隔离及双客户端移动画面回归。
