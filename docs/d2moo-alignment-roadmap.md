...
| P0 | Monster construction and level stats | CUSTOM | `ServerEntityFactory` currently builds monsters with expansion=true, difficulty=Normal, `MonStats.Level[0]`, and does not initialize native `STAT_MONSTER_PLAYERCOUNT`; `MonsterStatsCalculator` retains fabricated growth fallback | `D2Game/MONSTER/Monster.cpp`, `D2Common/DataTbls/MonsterTbls.cpp` |
