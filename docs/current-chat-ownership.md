...
- 分支：`master`
- 当前功能提交：本文件所在 `HEAD`（A1Q5 Countess 多人资格、幂等与重连收尾）
- 上一确认基线：`9958680c`（确认 P2 实际执行阶段与剩余边界）
...
当前目标是 **A1Q6 Andariel 多人/重连收尾**：核对整个 Act I 的队伍资格、Warriv
奖励/过幕状态、重复请求、跨幕排除、固定种子与断线重连恢复；完成后处理对象
`stateFlags` 客户端表现。战斗
