...
- 分支：`master`
- 当前功能提交：本文件所在 `HEAD`（A1Q6 Andariel 多人资格、Warriv 幂等与重连收尾）
- 上一功能基线：`24237b51`（A1Q5 Countess 多人任务与重连收尾）
...
当前目标是 **对象 `stateFlags` 客户端表现收尾**：将动态对象状态映射到客户端
`Interactable`，验证箱子、门、陷阱、神殿等激活/耗尽状态；随后继续第一章剩余脚本和
战斗/物品严格验收。战斗
