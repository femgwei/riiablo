...
public class ItemDurabilityManager {
  ...
  /** Native ITEMS_UpdateDurability probabilities (percent per successful hit). */
  public static final int WEAPON_DURABILITY_CHANCE = 4;
  public static final int ARMOR_DURABILITY_CHANCE = 10;
   ...
   * 消耗武器耐久度（攻击时调用）
   * 
   * <p>参考 D2MOD ITEMS_UpdateDurability. The native routine rolls 4% for
   * weapons and 10% for throwable/armor paths; this method's boolean keeps the
   * old API while exposing those native probabilities.
   * 
   * @param item 武器物品
  ...
  public boolean drainWeaponDurability(Item item, boolean isMelee) {
    ...
    // 概率检查
    // The legacy boolean identifies melee/ranged callers but does not change
    // the native weapon chance; throwable items have a separate native path.
    int chance = nativeDurabilityChance(false, false, true);
    ...
    for (int i = 0; i < ARMOR_DURABILITY_WEIGHTS.length; i++) {
      ...
      if (armorItems[currentIndex] != null) {
        if (randomWeight < ARMOR_DURABILITY_WEIGHTS[currentIndex].weight) {
          if (MathUtils.random(99) < ARMOR_DURABILITY_CHANCE
              && drainDurability(armorItems[currentIndex], 1)) {
  ...
  private boolean isIndestructible(Item item) {
    ...
    // Native STAT_ITEM_INDESTRUCTIBLE is an aggregate property (typically
    // generated by an affix), not a field in Items.txt.
    if (item.attrs != null) {
      StatRef indestructible = item.attrs.get(Stat.item_indesctructible);
      if (indestructible != null && indestructible.asInt() > 0) return true;
    }
  ...
  private void onItemBroken(Item item) {
    ...
    // D2 keeps an ethereal item in the inventory when broken; it becomes
    // unusable until a replenishing/native repair effect restores it.
    item.flags |= Item.ITEMFLAG_BROKEN;
  ...
  static int nativeDurabilityChance(boolean armor, boolean throwable, boolean expansion) {
    if (armor) return ARMOR_DURABILITY_CHANCE;
    if (throwable && expansion) return ARMOR_DURABILITY_CHANCE;
    if (throwable) return 0;
    return WEAPON_DURABILITY_CHANCE;
  }
}
