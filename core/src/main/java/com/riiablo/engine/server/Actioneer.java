  ...
  private void srvdofunc(int entityId, int srvdofunc, int targetId, Vector2 targetVec) {
    ...
    switch (srvdofunc) {
      ...
      case 30: { // native Necromancer curse dispatcher
        // ServerSkillSystem owns the shared data-driven SrvDo030 area path
        // for both players and monsters. Keeping a second name-based single
        // target implementation here caused divergent state/stat payloads.
        log.debug("[NECROMANCER_CURSE] phase=keyframe source={} delegated=ServerSkillSystem",
            entityId);
        break;
      }
