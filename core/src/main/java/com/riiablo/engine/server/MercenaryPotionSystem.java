...
public final class MercenaryPotionSystem extends BaseSystem {
  ...
  public boolean useOnMercenary(int playerId, Item potion) {
    if (playerId < 0 || !mPlayer.has(playerId) || potion == null
        || potion.location != Location.CURSOR || !isHealingPotion(potion)) return false;
    Player player = mPlayer.get(playerId);
    CharData character = player == null ? null : player.data;
    if (character == null || !character.isExpansion()) return false;
    ...
    for (int i = 0; i < entities.size(); i++) {
      ...
      if (hp == null || max == null || max.asFixed() <= hp.asFixed()) return false;
      float healed = Math.min(max.asFixed(), hp.asFixed() + healingAmount(potion));
      if (healed <= hp.asFixed()) return false;
      hp.set(healed);
